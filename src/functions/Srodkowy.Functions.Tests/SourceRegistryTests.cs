...
public sealed class SourceRegistryTests
{
    ...
    {
        SourceRegistry.All.Should().HaveCount(19);
    }

    [Fact]
    public void Registry_should_keep_both_camps_represented()
    {
        SourceRegistry.All.Count(source => source.Camp == SourceCamp.Left).Should().Be(11);
        SourceRegistry.All.Count(source => source.Camp == SourceCamp.Right).Should().Be(8);
    }
}
